...
export class GeminiReportAnalysisProvider implements ReportAnalysisProvider {
  ...
  async analyzeReport(input: ReportAnalysisInput): Promise<ReportAnalysisResult> {
    if (!this.apiKey || this.apiKey === "mock-ai-key" || this.apiKey === "invalid-key") {
      const err = new Error("Gemini authentication or missing API key") as Error & {
...
Return JSON matching these keys:
{
  ...
  "needsManualReview": boolean
    ...
    try {
      ...
      if (!response.ok) {
        const err = new Error(`Gemini HTTP error ${response.status}`) as Error & {
          errorCode?: SanitizedAiErrorCode;
        };
      ...
      if (!rawText) {
        const err = new Error("Gemini returned empty response body") as Error & {
          errorCode?: SanitizedAiErrorCode;
        };
      ...
      try {
        ...
        const err = new Error("Gemini response is not valid JSON") as Error & {
          errorCode?: SanitizedAiErrorCode;
        };
        err.errorCode = "INVALID_PROVIDER_RESPONSE";
        throw err;
      ...
      if (!validation.success) {
        const err = new Error(`Gemini JSON schema validation failed: ${validation.error.message}`) as Error & {
          errorCode?: SanitizedAiErrorCode;
        };
      ...
      if (err instanceof Error && err.name === "AbortError") {
        const timeoutErr = new Error("Gemini request timed out") as Error & {
          errorCode?: SanitizedAiErrorCode;
        };
